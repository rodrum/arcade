program complete_ecmwf
! ----------------------------------------------------------
! This program uses the data obtained from ECMWF (height, 
! temperature, zonal wind, and meridonial wind) and adds
! the columns of density and pressure with the simplest
! approximation.
!
! Uses latest MSIS2.0 and HWM14.
!
! by RSDNL @ West Campus
! 2022-02-24
! ----------------------------------------------------------
...
100 format (A28, I3.3, A1, I5.5, A1, I4.4, A4)
200 format (A22, I3.3, A1, I5.5, A1, I4.4, A4)

do idoy = 1, size(doy_vec)
    do isou = 1, num_sou
        do ista = 1, num_sta
            !=== Climatologies as in sph case
            write(nodes_in, 100) "../output/nodes/nodes_climt_", doy_vec(idoy), "_",  &
                                                          isou, "_",           &
                                                          ista, ".txt"
            ...
            write(nodes_out, 100) "../output/nodes/descr_climt_", doy_vec(idoy),     &
                                                           "_", isou,          &
                                                           "_", ista, ".txt"
            ...
            !=== ECMWF completion
            write(nodes_in, 100) "../output/nodes/nodes_ecmwf_", doy_vec(idoy), "_",  &
                                              isou, "_",           &
                                              ista, ".txt"
            write(*,*) "Reading ", trim(nodes_in)
            open (10, FILE=trim(nodes_in), STATUS="OLD", ACTION="read",        &
            ...
            write(ecmwf_in, 200) "../output/ecmwf/ecmwf_", doy_vec(idoy), "_",  &
                                              isou, "_",           &
                                              ista, ".dat"
            ...
            write(nodes_out, 100) "../output/nodes/descr_ecmwf_", doy_vec(idoy),     &
                                                           "_", isou,          &
                                                           "_", ista, ".txt"
